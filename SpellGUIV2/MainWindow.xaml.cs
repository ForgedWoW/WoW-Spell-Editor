...
namespace SpellEditor
{
    partial class MainWindow
    {
        ...
        private void RefreshAllUIElements()
        {
            ...
            // custom procs
            string[] customAuraStates = { " Power Below 50%"};

            foreach (string casterAuraState in customAuraStates) { CasterAuraState.Items.Add(casterAuraState); }
        ...
        private async void Button_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                uint oldIDIndex = selectedID;

                if (selectedID == default || selectedID == 0)
                {
                ...
                if (oldIDIndex != uint.MaxValue)
                {
                    ...
                    for (int i = 1; i < row.Table.Columns.Count; ++i)
                        str.Append($", \"{row[i].ToString().Replace("\"", "\\\"")}\"");
                    str.Append(")");
